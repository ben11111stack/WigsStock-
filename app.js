...
function statusChangesTable() {
  ...
  const fmtAt = (t) => { if (!t) return '—'; const d = new Date(t); return `${p2(d.getDate())}/${p2(d.getMonth() + 1)} ${p2(d.getHours())}:${p2(d.getMinutes())}`; };
...
function dataStatusChanges() {
  ...
  Object.keys(ov).sort((a, b) => a.localeCompare(b, undefined, { numeric: true })).forEach(bc => {
    ...
    rows.push([bc, wigName(bc), statusLabel(state.inventory[bc] || ''), statusLabel(ov[bc]),
      m.by || '', at ? `${p2(at.getDate())}/${p2(at.getMonth() + 1)}/${at.getFullYear()} ${p2(at.getHours())}:${p2(at.getMinutes())}` : '']);
  });
  return rows;
}
