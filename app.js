...
async function pullCloud(forceFull) {
  if (!cloudEnabled()) return;
  try {
    const full = !!forceFull || !(state.cloudSince > 0);
    const base = cloudBase() + (full ? '/api/scans?count_id=' : '/api/changes?count_id=') +
      encodeURIComponent(state.countId);
    const url = full ? base :
      base + '&since=' + encodeURIComponent(state.cloudSince || 0) +
      '&settings_since=' + encodeURIComponent(state.lastSettingsAt || 0) +
      '&reset_seen=' + encodeURIComponent(state.lastResetSeen || 0);
    ...
    if (full || data.full) {
      ...
      if (data.devices !== undefined) state.cloudDevices = data.devices || 0;
    } else {
      // Delta payloads contain only changed barcodes. A zero total means all
      // stations now report zero for that barcode, so remove it from the merged view.
      for (const [barcode, total] of Object.entries(data.scans || {})) {
        if (Number(total) > 0) state.cloudScans[barcode] = Number(total);
        else delete state.cloudScans[barcode];
      }
      for (const barcode of Object.keys(data.scans || {})) {
        if (data.detail && data.detail[barcode]) state.cloudDetail[barcode] = data.detail[barcode];
        else delete state.cloudDetail[barcode];
      }
    }
    if (data.version) state.cloudSince = Number(data.version) || state.cloudSince || 0;
...
function setCloudStatus(kind, data) {
  ...
  const shownBarcodes = data && data.barcodes !== undefined ? data.barcodes : Object.keys(state.cloudScans || {}).length;
  const shownDevices = data && data.devices !== undefined ? data.devices : (state.cloudDevices || 0);
  const extra = (kind === 'ok' && data) ? ` · ${shownBarcodes} ברקודים · ${shownDevices} עמדות` : '';
  const pend = Object.keys(state.dirty).length;
...
function applyCloudConfig() {
  // URL/count changes point at a different dataset. The next request must be a
  // full baseline before cheap deltas can resume.
  state.cloudSince = 0;
  save();
  if (cloudEnabled()) {
    markAllDirty();
