...
function notifyUnknown(code) {
  ...
  uiConfirm('הברקוד ' + code + ' לא נמצא בקובץ המלאי.\nייתכן שזו טעות סריקה.', {
    title: 'ברקוד לא מוכר', danger: true, confirmText: 'מחק סריקה', cancelText: 'השאר בכל זאת'
  }).then(discard => { if (discard) unrecordScan(code); });
}
...
function applyScanStatusOverride(bc, status) {
  ...
  if (state.inventory[bc] === status) delete state.statusOverrides[bc];  // matches the sheet → no override
