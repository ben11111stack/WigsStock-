...
- **גיבוי + שחזור** — `doResetScans()` (app.js) לוקח snapshot של `effectiveScans()` (סה"כ ממוזג מכל העמדות) **לפני** המחיקה ושומר ב-`state.resetHistory` (מקומי ל-localStorage, cap 30). כל רשומה: `{id, at, by (שם העמדה/משתמש), countId, totalScanned, totalCount, scans:{bc:count}, names, statusOverrides}`. האישור הוא `uiConfirm` עם `danger` + אזהרה חזקה. `restoreReset(id)` בונה מחדש את `state.scans`, מסמן `dirty`, ומריץ `schedulePush()` כדי להחזיר את הספירות לענן ולכל העמדות; `renderResetHistory()` מציג את הרשימה (שחזר/מחק). **הערה:** ההיסטוריה נשמרת רק במכשיר שביצע את האיפוס (לא מסונכרנת), ואחרי שחזור כל הספירות מיוחסות למכשיר המשחזר.
- בדיקות: `node tests/run.js` → 48 passed. נבנה `wigsstock-app.html`, `sw.js` CACHE = `wigsstock-v28`.
